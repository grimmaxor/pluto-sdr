...
# 256QAM Gray Map (16 levels per axis)
_GRAY_MAP_256 = {
    (0,0,0,0): -15, (0,0,0,1): -13, (0,0,1,1): -11, (0,0,1,0): -9,
    (0,1,1,0): -7,  (0,1,1,1): -5,  (0,1,0,1): -3,  (0,1,0,0): -1,
    (1,1,0,0): 1,   (1,1,0,1): 3,   (1,1,1,1): 5,   (1,1,1,0): 7,
    (1,0,1,0): 9,   (1,0,1,1): 11,  (1,0,0,1): 13,  (1,0,0,0): 15
}
_GRAY_INV_256 = {v: k for k, v in _GRAY_MAP_256.items()}
...
def bits_to_symbols(bits):
    ...
    b = bits.reshape(-1, 8)
    i_levels = np.array([_GRAY_MAP_256[(int(r[0]), int(r[1]), int(r[2]), int(r[3]))] for r in b], dtype=np.float32)
    q_levels = np.array([_GRAY_MAP_256[(int(r[4]), int(r[5]), int(r[6]), int(r[7]))] for r in b], dtype=np.float32)
    return ((i_levels + 1j * q_levels) / np.sqrt(170)).astype(np.complex64)


def symbols_to_bits(syms):
    s = syms * np.sqrt(170)
    def demap_axis(v):
        lv = np.clip(np.round((v + 15) / 2).astype(int) * 2 - 15, -15, 15)
        out = np.zeros((len(v), 4), dtype=np.uint8)
        for k in range(len(lv)):
            out[k] = _GRAY_INV_256[int(lv[k])]
        return out
...
def packet_to_iq(pkt_bytes):
    """Real-BPSK preamble + 256QAM payload -> shaped IQ for non-cyclic transmission."""
...
def _cfo_variants(iq):
    ...
    # 256QAM is incredibly phase-sensitive. We yield tiny micro-offsets 
...
def iq_to_packets(iq):
    ...
    for corrected_iq in _cfo_variants(iq):
        ...
        mag  = np.abs(corr)
        thr  = PREAMBLE_LEN * 0.4    # lowered threshold for 256QAM peak-scaling
        ...
        for c in cand:
            ...
            # Amplitude Recovery (critical for 256QAM)
...
def setup_pluto():
    ...
    print(f"[✓] Connected. Carrier {args.freq/1e6:.3f} MHz, 256QAM, SPS={args.sps}")
    return sdr
...
def receiver_main(sdr):
    ...
    print("[RX] Listening for 256QAM stream... (Ctrl+C to stop)")
