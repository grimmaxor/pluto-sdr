...
# MUST be small enough that several whole packet copies fit in one RX buffer
# (65536 samples). At worst case BPSK/SPS16, 64 bytes gives ~6 copies/buffer
# so the random RX capture window always contains at least one uncut copy.
# Larger chunks (e.g. 256) only fit ~1.9 copies -> packet split at wrap-around
# -> DATA never decodes even though tiny control packets do.
CHUNK_BYTES = 64
...
def packet_to_iq_fill(pkt_bytes):
    ...
    # Tile a WHOLE number of complete packets (never cut mid-packet),
    # then pad the remainder with silence. This guarantees every repeated
    # copy is intact so the RX scan always finds a clean packet.
    plen = len(shaped)
    if plen >= TX_BUFFER_SIZE:
        # Single packet bigger than buffer — shouldn't happen for our sizes,
        # but handle gracefully by sending as-is (truncated).
    ...
    iq_full  = np.concatenate([body, pad])
    return iq_full.astype(np.complex64)


def _barker_sync_bits():
    """The sync pattern as a +1/-1 bit-symbol array (Barker tiled x2)."""
    bbits = ((1 - BARKER_13) / 2).astype(np.uint8)
    sync_bits = np.tile(bbits, 2)
    # back to +1/-1 symbol signs for correlation
    return (1.0 - 2.0 * sync_bits.astype(np.float32))


SYNC_SIGNS = _barker_sync_bits()   # length 26


def _packets_from_symbol_stream(syms):
    """
    Given recovered complex symbols, find the Barker sync via bit-level
    correlation on the I-rail signs, then read packets starting right after
    each sync hit. Returns list of packet dicts.
    """
    found = {}
    # Use sign of real part for sync detection (works for BPSK & QPSK I-rail)
    signs = np.sign(syms.real).astype(np.float32)
    if len(signs) < len(SYNC_SIGNS) + 64:
        return found

    corr = np.correlate(signs, SYNC_SIGNS, mode='valid')
    thr  = len(SYNC_SIGNS) * 0.8
    # candidate sync positions (strong correlation, either polarity)
    cand = np.where(np.abs(corr) > thr)[0]

    for c in cand:
        inverted = corr[c] < 0          # constellation flipped
        data_sym_start = c + len(SYNC_SIGNS)
        data_syms = syms[data_sym_start:]
        if len(data_syms) < 16 * 8 // BITS_PER_SYMBOL:
            continue
        if inverted:
            data_syms = -data_syms
        bits = symbols_to_bits(data_syms)
        nb   = len(bits) // 8
        if nb < 16:
            continue
        raw = bytes(np.packbits(bits[:nb * 8]))
        # packet starts at byte 0 of the post-sync stream
        pkt = parse_packet(raw, 0)
        if pkt:
            found[(pkt['type'], pkt['seq'])] = pkt
        else:
            # try a small byte slip in case sync landed +-1 symbol off
            for slip in (1, -1, 2, -2):
                ss = data_sym_start + slip
                if ss < 0 or ss >= len(syms):
                    continue
                ds = syms[ss:]
                if inverted:
                    ds = -ds
                b2 = symbols_to_bits(ds)
                n2 = len(b2) // 8
                if n2 < 16:
                    continue
                r2 = bytes(np.packbits(b2[:n2 * 8]))
                p2 = parse_packet(r2, 0)
                if p2:
                    found[(p2['type'], p2['seq'])] = p2
                    break
    return found
...
def iq_to_packets(iq):
    ...
    Tries CFO none / FFT / PLL. Single matched filter (TX already shaped;
    we just sample at the symbol centres). Barker bit-correlation gives
    exact alignment so packets read directly without byte-scanning.
    """
    ...
    # Group delay of the TX pulse-shaping filter — sample at its centre.
    delay = len(FILT) // 2

    for corrected in _cfo_variants(iq):
        # Single matched filter to maximise SNR at symbol centre.
        ...
        for rot in _rotations():
            ci = filtered.real * rot[0] - filtered.imag * rot[1]
            cq = filtered.real * rot[1] + filtered.imag * rot[0]
            csig = (ci + 1j * cq).astype(np.complex64)

            # Sample at every timing offset around the expected delay
            for toff in range(SAMPLES_PER_SYMBOL):
                start = (delay + toff) % SAMPLES_PER_SYMBOL
                syms  = csig[start::SAMPLES_PER_SYMBOL]
                if len(syms) < 64:
                    continue
                pkts = _packets_from_symbol_stream(syms)
