#!/usr/bin/env python3
"""
ADALM Pluto — Live Video Streaming over 16QAM (STABLE + OVER-THE-AIR AUTO-CAL)
=============================================================================

Same DSP pipeline as pluto_video_stream_16qam_stable.py (RRC + RS FEC +
decision-directed equalizer + fast real-time demod), but with an over-the-air
TX<->RX auto-calibration handshake so the link can be brought up on air without
hand-tuning --rx-gain / --tx-atten.

The video stream itself is simplex (TX -> RX).  The reverse channel (RX -> TX)
is used ONLY during the brief calibration handshake, which runs half-duplex on
the streaming frequency before streaming begins:

  Phase A — RX gain:   TX beacons at a fixed cal power; RX sweeps its RX gain to
                       decode the beacon, then reports GAIN_OK.
  Phase B — TX power:  TX sweeps attenuation weak->strong; RX echoes each PWR
                       probe it decodes; TX locks the first power that gets
                       through (+ a small margin).
  Phase C — READY:     bidirectional READY handshake, then both sides drop into
                       one-way streaming.

Start BOTH sides at about the same time (within ~30 s) so the handshake lines up.

Usage:
  Pluto 1 (Sender):   python3 pluto_video_stream_16qam_stable_auto.py --role tx --input my_video.mp4
  Pluto 2 (Receiver): python3 pluto_video_stream_16qam_stable_auto.py --role rx
  Skip calibration:   add --skip-cal --rx-gain N --tx-atten N (use known-good values)
  Software self-test: python3 pluto_video_stream_16qam_stable_auto.py --role test
"""
...
# Control frames reuse the normal packet format; the control "type" is encoded in
# the high range of the 4-byte seq field so it can never collide with the video
# sequence numbers (which count up from 1).
CTRL_SEQ_BASE  = 0xCA000000
CAL_TONE       = 0          # TX -> RX beacon used while RX sweeps its gain
CAL_GAIN_OK    = 1          # RX -> TX "I locked my RX gain"
CAL_PWR        = 3          # TX power probe (TX -> RX) / its echo (RX -> TX)
CAL_READY      = 4          # bidirectional "calibration done" handshake

CAL_TX_ATTEN    = -30                      # TX power while RX sweeps gain (weak/safe)
CAL_ACK_ATTEN   = -10                      # RX's TX power for cal acks (reverse link uncal'd -> fairly strong)
TX_ATTEN_SWEEP  = list(range(-40, 1, 5))   # -40..0 dB, weak -> strong
CAL_PWR_MARGIN  = 5                        # extra dB of TX power for stability
...
#  AUTO-CALIBRATION
# ═══════════════════════════════════════════════════════════════════════════════
# ─── control-frame helpers (reuse the normal packet path) ──────────────────────
def _cal_build(ctrl_type, value=0):
    return build_packet(CTRL_SEQ_BASE + ctrl_type, struct.pack('>h', int(value)))

def _cal_match(pkt, want_type):
    """Return the int value if pkt is a control frame of want_type, else None."""
    s = pkt['seq']
    if s < CTRL_SEQ_BASE or (s - CTRL_SEQ_BASE) != want_type:
        return None
    pl = pkt['payload']
...
def _cal_listen(sdr, want_type, timeout=1.5):
    ...
    while time.time() < deadline:
        ...
        for pkt in iq_to_packets(rx):
            v = _cal_match(pkt, want_type)
            if v is not None:
                return v
    return None


def _sweep_rx_gain_beacon(sdr):
    """Sweep RX gain against the TX CAL_TONE beacon; return the best gain."""
    print("\n[RX] Phase A: sweeping RX gain against TX beacon ...")
    print(f"  {'Gain':>5}  {'Peak':>6}  {'ADC%':>5}  {'Beacons':>8}")
    candidates = []
    for g in range(0, 75, 5):
        ...
        for _ in range(3):
            try:
                ...
                for pkt in iq_to_packets(rx):
                    if _cal_match(pkt, CAL_TONE) is not None:
                        dec += 1
                        break
            except Exception: pass
        adc = pk / 2896 * 100
        print(f"  {g:>5}  {pk:>6.0f}  {adc:>4.0f}%  {dec:>8}")
        if adc < 95 and dec > 0:               # decoded beacon, not clipping
            candidates.append((dec, g, adc))
    if candidates:
        # prefer reliable decodes, then higher gain (more margin) without clipping
        candidates.sort(key=lambda c: (c[0], c[1]))
        best = candidates[-1][1]
        print(f"[RX] ✓ RX gain = {best} dB")
        return best
    print(f"[RX] ! No beacon decoded — falling back to --rx-gain {args.rx_gain} dB")
    return int(args.rx_gain)


# ═══════════════════════════════════════════════════════════════════════════════
#  OVER-THE-AIR AUTO-CALIBRATION  (half-duplex handshake before streaming)
# ═══════════════════════════════════════════════════════════════════════════════
def calibrate_tx(sdr):
    print("\n" + "=" * 54)
    ...
    t_end  = time.time() + 90
    while time.time() < t_end:
        _cal_tx(sdr, CAL_TONE, hold=1.5)
        if _cal_listen(sdr, CAL_GAIN_OK, timeout=1.2) is not None:
            got_ok = True
            break
    print("[TX] " + ("✓ RX reported gain lock." if got_ok
                     else "! RX gain-OK not heard; continuing anyway."))

    # Phase B: sweep TX power weak -> strong; RX echoes the first level it hears.
    print("\n[TX] Phase B: negotiating TX power (weak -> strong).")
    chosen = None
    for atten in TX_ATTEN_SWEEP:
        sdr.tx_hardwaregain_chan0 = atten
        _cal_tx(sdr, CAL_PWR, value=atten, hold=1.2)
        echo = _cal_listen(sdr, CAL_PWR, timeout=1.8)
        print(f"  TX atten {atten:>4} dB -> {'echo OK' if echo is not None else 'no echo'}")
        if echo is not None:
            chosen = atten
            break
    if chosen is None:
        chosen = 0
        print("[TX] ! No echo at any power; using max power (0 dB).")
    else:
        chosen = min(0, chosen + CAL_PWR_MARGIN)   # margin for stability
    sdr.tx_hardwaregain_chan0 = chosen
    print(f"[TX] ✓ TX attenuation = {chosen} dB")

    # Phase C: bidirectional READY handshake.
    print("\n[TX] Phase C: confirming link ...")
    for _ in range(12):
        _cal_tx(sdr, CAL_READY, hold=0.9)
...
def calibrate_rx(sdr):
    print("\n" + "=" * 54)
    print("  RX — over-the-air auto-calibration")
    print("=" * 54)
    ...
    t_end = time.time() + 40
    while time.time() < t_end:
        _cal_tx(sdr, CAL_GAIN_OK, hold=1.0)
        if _cal_listen(sdr, CAL_PWR, timeout=1.0) is not None:
            break

    # Phase B: echo every PWR probe we decode so the TX can lock its power.
    print("[RX] Phase B: echoing TX power probes ...")
    t_end = time.time() + 60
    while time.time() < t_end:
        v = _cal_listen(sdr, CAL_PWR, timeout=2.0)
        if v is not None:
            _cal_tx(sdr, CAL_PWR, value=v, hold=1.2)
        if _cal_listen(sdr, CAL_READY, timeout=0.6) is not None:
            break

    # Phase C: READY handshake.
    print("[RX] Phase C: confirming link ...")
    for _ in range(12):
        if _cal_listen(sdr, CAL_READY, timeout=1.5) is not None:
...
def main():
    ...
    if ROLE == 'tx':
        if args.skip_cal:
            print(f"[*] Skipping calibration — TX atten {args.tx_atten} dB")
            sdr.tx_hardwaregain_chan0 = int(args.tx_atten)
        else:
            calibrate_tx(sdr)
        sender_main(sdr)
