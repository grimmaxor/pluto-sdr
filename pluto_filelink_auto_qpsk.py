...
AUTO-CALIBRATION
----------------
On startup (unless --skip-cal), the two radios calibrate the link exactly
like pluto_autolink.py: master drives a 4-phase sequence over the air to
auto-set RX gain on BOTH radios and negotiate TX power, then the file
transfer begins. To skip and use fixed values:
  --skip-cal --rx-gain 10 --tx-atten -30
Providing BOTH --rx-gain and --tx-atten also skips calibration automatically.
"""
...
parser.add_argument('--skip-cal',  action='store_true', help='skip auto-calibration (use --rx-gain/--tx-atten or defaults)')
...
RX_BUFFER_SIZE     = 262144   # larger than TX so any window holds >=3 full packets
MOD                = args.mod
...
PKT_CAL_TONE = 0x10   # beacon used during RX-gain sweep
PKT_GAIN_OK  = 0x11   # "I locked my RX gain"
PKT_YOURTURN = 0x12   # "your turn to transmit the beacon"
PKT_PWR      = 0x13   # power probe / echo
PKT_READY    = 0x14   # calibration handshake complete
...
RX_GAIN_SWEEP  = list(range(0, 74, 3))     # 0..73 dB, low -> high
TX_ATTEN_SWEEP = list(range(-80, 1, 5))    # -80..0 dB, weak -> strong
GAIN_CONFIRM   = 3                          # decodes needed to accept a gain step
CAL_FIXED_ATTEN = -30                       # TX power used while sweeping gain
...
def packet_to_iq_fill(pkt_bytes):
    """
    Build: [BPSK preamble (Barker x3)] + [payload symbols in chosen MOD]
    The PREAMBLE IS ALWAYS BPSK (real +-1) regardless of MOD. This makes sync
    detection rotation-tolerant and identical across BPSK/QPSK/16QAM.
    Pulse-shaped, then tiled to fill the whole TX buffer (no mid-packet cuts).
    """
    # --- Preamble: Barker-13 repeated 3x, as real BPSK symbols ---
    preamble = np.tile(BARKER_13, 3).astype(np.complex64)   # +-1 real, len 39

    # --- Payload: packet bytes in the selected modulation ---
    ...
    # Tile WHOLE packets only, pad remainder with silence.
    plen = len(shaped)
...
# Preamble as real +-1 signs for correlation (Barker-13 x3 = 39 symbols)
PREAMBLE_SIGNS = np.tile(BARKER_13, 3).astype(np.float32)
PREAMBLE_LEN   = len(PREAMBLE_SIGNS)
# Number of payload SYMBOLS that follow the preamble depends on packet size,
# but we don't need to know it for sync — we read until the stream ends.


def _packets_from_symbol_stream(syms):
    """
    Recovered complex symbols -> find BPSK preamble via correlation on the
    REAL rail -> derotate using the preamble's own phase -> demap payload.

    Because the preamble is real BPSK, its correlation peak directly reveals
    BOTH the timing AND the residual carrier phase, letting us derotate the
    payload exactly. This resolves QPSK's 4-fold phase ambiguity for free.
    """
    found = {}
    n = len(syms)
    if n < PREAMBLE_LEN + 32:
        return found

    # Correlate the COMPLEX symbol stream against the real preamble.
    # corr[c] is complex: its magnitude peaks at the preamble location, and
    # its angle gives the residual phase rotation to undo.
    # np.correlate with complex input conjugates the second arg internally,
    # but our preamble is real so it's just a sliding dot product.
    corr = np.correlate(syms, PREAMBLE_SIGNS.astype(np.complex64), mode='valid')
    mag  = np.abs(corr)
    thr  = PREAMBLE_LEN * 0.6     # max possible is PREAMBLE_LEN (unit symbols)
    ...
    for c in cand:
        # Residual phase from the preamble correlation
        phi = np.angle(corr[c])
        derot = np.exp(-1j * phi)

        data_start = c + PREAMBLE_LEN
        data_syms  = syms[data_start:] * derot
        if len(data_syms) < (16 * 8) // BITS_PER_SYMBOL:
            continue

        # Demap and try to parse, allowing a small symbol slip for safety
...
def iq_to_packets(iq):
    """
    Demodulate an RX buffer and extract ALL valid packets.
    The real-BPSK preamble carries its own phase reference, so we do NOT
    need separate rotation hypotheses — the preamble correlation derotates
    the payload directly. We still try a coarse CFO correction in case the
    offset is large enough to smear the preamble across the buffer.
    """
    ...
    found = {}
    delay = len(FILT) // 2     # group delay of pulse-shaping filter
    ...
    for corrected in _cfo_variants(iq):
        ...
        filtered = (fi + 1j * fq).astype(np.complex64)

        # Sample at each timing phase around the filter group delay
        for toff in range(SAMPLES_PER_SYMBOL):
            start = (delay + toff) % SAMPLES_PER_SYMBOL
            symstream = filtered[start::SAMPLES_PER_SYMBOL]
            if len(symstream) < PREAMBLE_LEN + 32:
                continue
            pkts = _packets_from_symbol_stream(symstream)
            for k, v in pkts.items():
                found[k] = v
        # Early exit if we already found something on this CFO variant
        if found:
            break
...
def _cfo_variants(iq):
    """
    Yield CFO-corrected versions of the buffer.
    Order matters: try 'none' first (cheap & usually fine on coax / short
    range), then a coarse FFT estimate. The preamble correlation handles
    residual phase, so we don't need the heavy per-sample PLL here.
    """
    yield iq  # no correction

    # Coarse CFO estimate via Mth-power FFT (M=2 BPSK, 4 QPSK/16QAM).
    # Normalise first to avoid float overflow on large magnitudes.
...
# ═══════════════════════════════════════════════════════════════════════════════
#  AUTO-CALIBRATION  (master drives, slave responds — runs over the radio link)
# ═══════════════════════════════════════════════════════════════════════════════
def _sweep_rx_gain(sdr, label):
    """
    Sweep our RX gain low->high while the OTHER side transmits PKT_CAL_TONE.
    Pick the gain that decodes the beacon reliably without ADC clipping.
    """
    print(f"\n[{label}] Sweeping RX gain against beacon...")
    ...
    for g in RX_GAIN_SWEEP:
        ...
        for _ in range(5):
            try:
                ...
                for pkt in iq_to_packets(rx):
                    if pkt['type'] == PKT_CAL_TONE:
                        decodes += 1
                        break
            except Exception:
                pass
    ...
    if not candidates:
        print(f"[{label}] No gain decoded the beacon — defaulting to 25 dB.")
        return 25
    candidates.sort(reverse=True)
    _, g, adc, dec = candidates[0]
    print(f"[{label}] \u2713 RX gain = {g} dB  (ADC {adc:.0f}%, {dec}/5 decodes)")
    return g


def calibrate_master(sdr):
    print("\n" + "="*54)
    print("  MASTER — auto-calibration")
    print("="*54)

    # Phase A: master transmits beacon so SLAVE can set its RX gain.
    print("\n[MASTER] Phase A: beaconing so slave can set RX gain.")
    set_tx_atten(sdr, CAL_FIXED_ATTEN)
    got_ok = False
    t_end  = time.time() + 60
    while time.time() < t_end:
        tx_packet(sdr, build_packet(PKT_CAL_TONE, 0, 0), hold=2.0)
        tx_silence(sdr)
        pkt = wait_for_packet(sdr, PKT_GAIN_OK, timeout=1.5)
        if pkt:
            got_ok = True
            break
    print("[MASTER] " + ("\u2713 Slave locked RX gain." if got_ok
                          else "Slave gain-OK not heard; continuing."))

    # Phase B: tell slave to beacon, master sweeps its own RX gain.
    print("\n[MASTER] Phase B: requesting slave beacon to set MY RX gain.")
    for _ in range(6):
        tx_packet(sdr, build_packet(PKT_YOURTURN, 0, 0), hold=0.5)
    tx_silence(sdr)
    rx_gain = _sweep_rx_gain(sdr, "MASTER")
    set_rx_gain(sdr, rx_gain)

    # Phase C: TX power negotiation (weak -> strong; slave echoes when heard).
    print("\n[MASTER] Phase C: negotiating TX power (high atten -> low atten).")
    chosen = None
    for atten in TX_ATTEN_SWEEP:
        set_tx_atten(sdr, atten)
        tx_packet(sdr, build_packet(PKT_PWR, 0, 0), hold=1.2)
        tx_silence(sdr)
        echo = wait_for_packet(sdr, PKT_PWR, timeout=1.8)
        print(f"  TX atten {atten:>4} dB -> {'echo OK' if echo else 'no echo'}")
        if echo:
            chosen = atten
            break
    if chosen is None:
        chosen = 0
        print("[MASTER] No echo at any level; using max power (0 dB).")
    else:
        chosen = min(0, chosen + 5)   # +5 dB margin for stability
    set_tx_atten(sdr, chosen)
    print(f"[MASTER] \u2713 TX attenuation = {chosen} dB")

    # Phase D: bidirectional READY handshake.
    print("\n[MASTER] Phase D: confirming link...")
    for _ in range(12):
        tx_packet(sdr, build_packet(PKT_READY, 0, 0), hold=1.0)
        tx_silence(sdr)
        if wait_for_packet(sdr, PKT_READY, timeout=1.5):
            print("[MASTER] \u2713 Link confirmed both ways!")
            break

    print(f"\n[MASTER] Calibration result: RX gain {rx_gain} dB, "
          f"TX atten {chosen} dB")
    return rx_gain, chosen


def calibrate_slave(sdr):
    print("\n" + "="*54)
    print("  SLAVE — auto-calibration")
    print("="*54)

    # Phase A: sweep OUR RX gain against master's beacon.
    print("\n[SLAVE] Phase A: sweeping RX gain against master beacon.")
    tx_silence(sdr)
    rx_gain = _sweep_rx_gain(sdr, "SLAVE")
    set_rx_gain(sdr, rx_gain)

    # Report gain locked until master moves on (asks us to beacon).
    print("[SLAVE] Reporting gain lock...")
    set_tx_atten(sdr, CAL_FIXED_ATTEN)
    for _ in range(8):
        tx_packet(sdr, build_packet(PKT_GAIN_OK, 0, 0), hold=1.0)
        tx_silence(sdr)
        if wait_for_packet(sdr, PKT_YOURTURN, timeout=1.0):
            break

    # Phase B: beacon so master can set its RX gain.
    print("\n[SLAVE] Phase B: beaconing for master's gain sweep.")
    t_end = time.time() + 30
    while time.time() < t_end:
        tx_packet(sdr, build_packet(PKT_CAL_TONE, 0, 0), hold=1.0)
        tx_silence(sdr)
        if wait_for_packet(sdr, PKT_PWR, timeout=0.8):
            break

    # Phase C: echo power probes back to master.
    print("\n[SLAVE] Phase C: echoing power probes.")
    set_tx_atten(sdr, CAL_FIXED_ATTEN)
    t_end = time.time() + 50
    while time.time() < t_end:
        if wait_for_packet(sdr, PKT_PWR, timeout=2.0):
            tx_packet(sdr, build_packet(PKT_PWR, 0, 0), hold=1.2)
            tx_silence(sdr)
        if wait_for_packet(sdr, PKT_READY, timeout=0.6):
            break
    ...
    print(f"\n[SLAVE] Calibration result: RX gain {rx_gain} dB, "
          f"TX atten {CAL_FIXED_ATTEN} dB")
    return rx_gain, CAL_FIXED_ATTEN
...
def main():
    ...
    # ── Auto-calibration (unless skipped or manually overridden) ──
    if args.skip_cal or (args.rx_gain is not None and args.tx_atten is not None):
        if args.skip_cal:
            print("[*] Skipping auto-calibration (--skip-cal).")
        else:
            print("[*] Using manual RX gain / TX atten (calibration skipped).")
        rx_gain  = args.rx_gain  if args.rx_gain  is not None else sdr.rx_hardwaregain_chan0
        tx_atten = args.tx_atten if args.tx_atten is not None else sdr.tx_hardwaregain_chan0
        set_rx_gain(sdr, rx_gain)
        set_tx_atten(sdr, tx_atten)
    else:
        print(f"\n*** ROLE: {ROLE.upper()} — make sure the OTHER PC is running "
              f"the opposite role. Calibration starts in 3s ***")
        time.sleep(3)
        if ROLE == 'master':
            rx_gain, tx_atten = calibrate_master(sdr)
        else:
            rx_gain, tx_atten = calibrate_slave(sdr)
