"""
ADALM Pluto — FDD RAW Image Transfer  (RX-only calibration variant)
====================================================================
Identical to pluto_image_fdd_raw.py except calibration only sweeps RX gain
and then fixes TX attenuation at TX_ATTEN_FINAL (-10 dB).  TX power
negotiation and link-confirm phases are removed.

  Pluto 1 (image SENDER) : python pluto_image_fdd_raw_rxcal.py --role tx --image photo.jpg
  Pluto 2 (image RECEIVER): python pluto_image_fdd_raw_rxcal.py --role rx --out-dir ./received
...
ap.add_argument('--skip-cal',  action='store_true',
                help='skip over-the-air calibration; use --rx-gain as-is')
args = ap.parse_args()
...
# Calibration
CAL_TX_ATTEN       = -30           # TX power while the partner sweeps RX gain
TX_ATTEN_FINAL     = -10           # fixed TX atten applied after RX cal
GAIN_STEP_BUFS     = 6
GAIN_CONFIRM       = 2
GAIN_RETRIES       = 8
...
# Calibration (both directions, before transfer)
PKT_CAL_TONE = 0x20
...
def setup_pluto():
    ...
    print(f"[✓] Connected.  TX {TX_FREQ/1e6:.3f} MHz   RX {RX_FREQ/1e6:.3f} MHz   "
          ...
          f"RXg={args.rx_gain} TXa={CAL_TX_ATTEN}")
    return sdr
...
# ═══════════════════════════════════════════════════════════════════════════════
#  AUTO-CALIBRATION  (RX gain sweep only — TX atten fixed at TX_ATTEN_FINAL)
# ═══════════════════════════════════════════════════════════════════════════════
...
def calibrate(sdr, label):
    print(f"\n{'='*54}\n  ROLE {label} — RX calibration (TX atten fixed at {TX_ATTEN_FINAL} dB)\n{'='*54}")
    rx_gain = calibrate_rx_gain(sdr, label)
    set_tx_atten(sdr, TX_ATTEN_FINAL)
    print(f"\n[{label}] Calibration result: RX gain {rx_gain} dB, TX atten {TX_ATTEN_FINAL} dB")
    return rx_gain, TX_ATTEN_FINAL
    ...
    for blk in range(nblk):
        ...
        while True:
            for seq in to_send:
                ...
                print_progress(blk + 1, nblk, "TX", f"block pkt {seq-lo+1}/{hi-lo}")
                t0 = time.time()
        ...
        while not stop.is_set():
            ...
            if f is not None:
                if f['type'] == PKT_DATA and lo <= f['seq'] < hi:
                    if f['seq'] not in buf:
                        buf[f['seq']] = f['payload']
                    print_progress(len(buf), total, "RX", f"block {blk+1}/{nblk}")
