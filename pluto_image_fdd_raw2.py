...
Canonical source: https://github.com/grimmaxor/pluto-sdr
  ...
  Pluto 1 (image SENDER) : python pluto_image_fdd_raw2.py --role tx --image photo.jpg
  Pluto 2 (image RECEIVER): python pluto_image_fdd_raw2.py --role rx --out-dir ./received
...
FIXES vs pluto_image_fdd_raw.py
--------------------------------
Fix 1 — TX/RX block desync deadlock:
  When the BACK packet for block N is dropped on the CTRL channel, TX times out
  and retransmits block N while RX has already advanced to block N+1.  TX ignores
  RX's REQ(N+1) and RX ignores TX's EOR(N) — permanent deadlock.
  Fix: in the TX WAIT loop, a REQ with seq > current block means RX advanced past
  us (our EOR reached it but BACK didn't reach us).  Treat it as implicit BACK and
  advance.  On the RX side, receiving EOR for a previous block while in a later
  block means we need to re-send BACK for the earlier block.

Fix 2 — Premature REQ at block start:
  last_req = 0.0 caused the periodic-REQ timer to fire on the very first loop
  iteration of every block, requesting ALL chunks before a single DATA packet was
  received.  Fix: initialise last_req = time.time() so the first REQ fires no
  earlier than REQ_INTERVAL after the block starts.

Fix 3 — Stale META packets starving the background listener at block 0:
  The ANNOUNCE phase leaves multiple decoded META packets in frame_q.  In the
  block 0 receive loop, each META hit acquires sdr_lock twice (to send GO), which
  blocks the background listener from calling sdr.rx() during the critical first
  seconds of block 0 — causing DATA packets to be missed.
  Fix (a): drain all stale META packets from frame_q before entering the block
  loop (they are useless once we have sent GO and are in receive mode).
  Fix (b): rate-limit in-loop GO responses so at most one GO is sent per second
  even if many META packets accumulate.
"""
    ...
    for blk in range(nblk):
        ...
        while True:
            ...
            while time.time() - t_wait < WAIT_TIMEOUT:
                ...
                # A REQ for block blk+1 (or later) means the receiver completed
                # this block and sent BACK, but the BACK was dropped on the CTRL
                # channel. Without this check TX stays on blk while RX is on
                # blk+1 — both ignoring each other's frames forever.
                    ...
                    if to_send:
                        ...
                        while time.time() - t_g < 0.6:
                            ...
                            if g2['type'] == PKT_REQ and g2['seq'] == blk:
                                ...
                                advanced = True; break
                            # FIX 1 also applies inside the grace window
...
def receiver_main(sdr):
    ...
    # ── FIX 3a: drain stale META packets before entering the block loop ──────
    # The ANNOUNCE phase leaves many decoded META copies in frame_q. In the
    # block 0 loop each META causes two sdr_lock acquisitions (to send GO),
    ...
    for blk in range(nblk):
        ...
        # last_req = 0.0 caused the periodic-REQ check to fire on the very
        # first loop iteration, flooding the CTRL channel with requests for
        # all chunks before a single DATA packet had been received.
        last_req = time.time()
        # ─────────────────────────────────────────────────────────────────────
            ...
            if f is not None:
                if f['type'] == PKT_DATA and lo <= f['seq'] < hi:
                    ...
                    if not missing:
                        ...
                        last_req = time.time()
                # ── FIX 1 (RX side): EOR for a previous block ───────────────
                # TX timed out and is retransmitting an old block while we are
                # already on a later one. Re-send BACK so TX can advance.
                elif f['type'] == PKT_EOR and f['seq'] < blk:
                ...
                # ─────────────────────────────────────────────────────────────
                    ...
                    # Without this, stale META in the queue causes repeated
                    # sdr_lock acquisitions that starve the background RX
                    # listener and cause DATA packets to be missed.
