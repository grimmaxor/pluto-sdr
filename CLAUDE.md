...
# FDD RAW v2 QPSK — same protocol/ARQ as raw2 but selectable --mod {bpsk,qpsk} (default qpsk, ~2x throughput)
...
| `pluto_image_fdd_raw2_qpsk.py` | FDD | **QPSK variant of `pluto_image_fdd_raw2.py`** (separate file; original left BPSK-only). Selectable `--mod {bpsk,qpsk}`, default **qpsk** (2 bits/symbol → ~2× throughput). DSP generalised from `pluto_filelink_auto_qpsk.py`: payload via `bits_to_symbols`/`symbols_to_bits`, both I/Q rails pulse-shaped, and a **complex** correlation against the always-real-BPSK Barker×3 preamble recovers timing **and** residual carrier phase → direct payload derotation (resolves QPSK's 4-fold ambiguity, no rotation search). `_cfo_variants` uses Mth-power 4 for QPSK. All framing/ARQ/calibration identical to `_raw2` (the three `_raw2` fixes carried over verbatim). **TX and RX must use the same `--mod`** (no in-band signalling). **16QAM intentionally excluded:** the BPSK preamble gives a phase but no amplitude reference, so multi-level constellations don't decode reliably (fails even noiseless in the offline self-test). Offline round-trip verified for bpsk+qpsk at noise σ=60. |
