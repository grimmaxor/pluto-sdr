...
ap.add_argument('--mod',       choices=['bpsk', 'qpsk', '16qam'], default='qpsk',
                help='payload modulation (preamble is always BPSK). TX and RX must match. '
                     '16QAM uses preamble correlation magnitude for amplitude recovery.')
...
def _packets_from_symbol_stream(syms):
    ...
    mag  = np.abs(corr)
    thr  = PREAMBLE_LEN * (0.4 if MOD == '16qam' else 0.6)
