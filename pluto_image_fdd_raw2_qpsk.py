"""
ADALM Pluto — FDD RAW Image Transfer (uncompressed, unlimited size, QPSK-capable)
=================================================================================
QPSK variant of pluto_image_fdd_raw2.py.  A dedicated one-way image link with a
reverse control channel, pure Python.

Canonical source: https://github.com/grimmaxor/pluto-sdr

  Pluto 1 (image SENDER) : python pluto_image_fdd_raw2_qpsk.py --role tx --image photo.jpg
  Pluto 2 (image RECEIVER): python pluto_image_fdd_raw2_qpsk.py --role rx --out-dir ./received

Both sides MUST use the same --mod (default qpsk); there is no in-band modulation
signalling.  Plain invocation on both radios is consistent.
...
MODULATION (this is what differs from the BPSK pluto_image_fdd_raw2.py)
-----------------------------------------------------------------------
Selectable --mod {bpsk,qpsk,16qam}, default QPSK (2 bits/symbol → ~2x the BPSK
throughput).  The PREAMBLE IS ALWAYS REAL BPSK (Barker-13 x3) regardless of the
payload modulation.  A *complex* correlation of the recovered symbol stream
against that real preamble yields BOTH the symbol timing AND the residual carrier
phase, so the payload is derotated directly — resolving QPSK's 4-fold phase
ambiguity for free, with no rotation/PLL search.  (Same pattern as
pluto_filelink_auto_qpsk.py.)  The framing, ARQ, threading, and FDD calibration
are modulation-agnostic and unchanged from pluto_image_fdd_raw2.py.
...
GAINS
-----
This script does NOT run the over-the-air calibration handshake unless asked.
Set RX gain / TX attenuation manually and tune live with pluto_gain_control.py
if the link is weak:  --skip-cal --rx-gain 40 --tx-atten -20
Develop CONDUCTED where possible: Pluto1 TX --[attenuator]--> Pluto2 RX, etc.

FIXES carried over from pluto_image_fdd_raw2.py
-----------------------------------------------
Fix 1 — TX/RX block desync deadlock (REQ(seq>blk) treated as implicit BACK;
        RX re-sends BACK for EOR of an already-completed block).
Fix 2 — Premature REQ at block start (last_req initialised to time.time()).
Fix 3 — Stale META starving the listener (single-pass snapshot drain + GO rate-limit).
See the original file's header for the full description of each.
"""
...
ap.add_argument('--mod',       choices=['bpsk', 'qpsk'], default='qpsk',
                help='payload modulation (preamble is always BPSK). TX and RX must match. '
                     '16QAM is intentionally not offered: the real-BPSK preamble carries a '
                     'phase reference but no amplitude reference, which a multi-level '
                     'constellation needs — only phase-only BPSK/QPSK decode reliably here.')
...
ap.add_argument('--blast',     action='store_true',
                help='DEBUG: handshake-free decode test. tx loops META+DATA forever (no GO/ARQ); '
                     'rx passively decodes and prints a stats line every 2 s. Implies --skip-cal.')
args = ap.parse_args()
...
# A packet must fit inside ONE TX buffer (with room to pad). The wire size depends
# on the modulation (more bits/symbol -> more bytes fit). Guard the user's --chunk.
_MAX_PKT_BITS = (TX_BUFFER_SIZE // SAMPLES_PER_SYMBOL) * BITS_PER_SYMBOL
_MAX_CHUNK    = (_MAX_PKT_BITS // 8) - 16 - 40                # minus header+crc+preamble margin
if CHUNK_BYTES > _MAX_CHUNK:
    print(f"[!] --chunk {CHUNK_BYTES} too large for SPS={SAMPLES_PER_SYMBOL} MOD={MOD}; "
...
# ─── MODULATION: bits <-> constellation symbols ───────────────────────────────
...
def packet_to_iq(pkt_bytes):
    """
    Real-BPSK preamble (Barker x3) + payload symbols in the chosen MOD ->
    pulse-shaped IQ, tiled with WHOLE copies only (+ silence pad) so every repeat
    in the cyclic buffer is intact and the RX scan always finds a clean packet.

    The preamble is ALWAYS real BPSK regardless of MOD, so its correlation peak on
    RX carries both timing and residual phase (see _packets_from_symbol_stream).
    """
...
def _cfo_variants(iq):
    """Yield CFO-corrected versions: none, then a coarse Mth-power FFT estimate."""
    yield iq

    nrm = iq / (np.max(np.abs(iq)) + 1e-9)
    power = 2 if MOD == 'bpsk' else 4    # Mth-power: 2 for BPSK, 4 for QPSK/16QAM
    sq  = nrm ** power
    n   = len(sq)
    fv  = np.fft.fft(sq); fv[0] = 0
    freqs = np.fft.fftfreq(n, d=1.0/SAMPLE_RATE)
    cfo = freqs[int(np.argmax(np.abs(fv)))] / power
    if abs(cfo) > 50:
...
def _packets_from_symbol_stream(syms):
    """
    Recovered complex symbols -> find the real BPSK preamble via COMPLEX
    correlation -> derotate the payload using the preamble's own phase -> demap.

    Because the preamble is real BPSK, the complex correlation peak directly
    reveals BOTH the timing (its magnitude) AND the residual carrier phase (its
    angle), letting us derotate the payload exactly. This resolves QPSK's 4-fold
    phase ambiguity for free.  Returns {(type,seq): pkt}.
    """
    found = {}
    n = len(syms)
    ...
    for c in cand:
        ...
        if len(syms[data_start:]) < (16 * 8) // BITS_PER_SYMBOL:
            continue
...
def setup_pluto():
    ...
    print(f"[✓] Connected.  TX {TX_FREQ/1e6:.3f} MHz   RX {RX_FREQ/1e6:.3f} MHz   "
          f"{MOD.upper()} SPS={SAMPLES_PER_SYMBOL}  chunk={CHUNK_BYTES}B  "
        ...
        while True:
            ...
            while time.time() - t_wait < WAIT_TIMEOUT:
                ...
                if f['type'] == PKT_BACK and f['seq'] == blk:
                    advanced = True
                    break
                # ── FIX 1: RX has advanced past this block ──────────────────
                if f['type'] == PKT_REQ and f['seq'] > blk:
                    print(f"\n[TX] REQ for block {f['seq']} while on block {blk} "
                          f"— BACK was dropped, advancing")
                    advanced = True
                    break
                # ────────────────────────────────────────────────────────────
                if f['type'] == PKT_REQ and f['seq'] == blk:
                    ...
                    if to_send:
                        ...
                        while time.time() - t_g < 0.6:
                            ...
                            if g2['type'] == PKT_REQ and g2['seq'] == blk:
                                ...
                                advanced = True; break
                            elif g2['type'] == PKT_REQ and g2['seq'] > blk:
                                print(f"\n[TX] REQ for block {g2['seq']} in grace window "
                                      f"— advancing from block {blk}")
                                advanced = True; break
...
def receiver_main(sdr):
    ...
    # ── FIX 3a: single-pass snapshot drain of stale META before the block loop ──
    _snap = []
    while True:
        try:
    ...
    for blk in range(nblk):
        lo, hi = block_range(blk, total)
        # ── FIX 2: start the REQ timer from now, not from epoch 0 ──
        last_req = time.time()
        # ── FIX 3b: rate-limit in-loop GO responses to stale META ──
        last_go_sent = 0.0
        while not stop.is_set():
            try:
            ...
            if f is not None:
                if f['type'] == PKT_DATA and lo <= f['seq'] < hi:
                    ...
                    if not missing:
                        ...
                        last_req = time.time()
                # ── FIX 1 (RX side): EOR for a previous block -> re-send BACK ──
                elif f['type'] == PKT_EOR and f['seq'] < blk:
                    print(f"\n[RX] Got EOR for old block {f['seq']} (on block {blk}) "
                          f"— re-sending BACK to unblock TX")
                    with sdr_lock:
                        tx_set(sdr, build_packet(PKT_BACK, f['seq'], nblk))
                    time.sleep(0.15)
                    with sdr_lock:
                        tx_set(sdr, build_packet(PKT_ACK, 0, 0))
                elif f['type'] == PKT_META and decode_meta(f['payload']):
                    # ── FIX 3b: rate-limit GO responses ──
                    if time.time() - last_go_sent >= 1.0:
                        for _ in range(2):
                            with sdr_lock:
                                tx_set(sdr, build_packet(PKT_GO, 0, total))
                            time.sleep(0.12)
                        last_go_sent = time.time()
...
# ═══════════════════════════════════════════════════════════════════════════════
#  BLAST  (DEBUG — handshake-free decode test, isolates the QPSK demod from ARQ)
# ═══════════════════════════════════════════════════════════════════════════════
def blast_sender(sdr):
    """Loop META + every DATA packet forever, ignoring the control channel."""
    if not os.path.isfile(args.image):
        print(f"[BLAST-TX] File not found: {args.image}"); return
    with open(args.image, 'rb') as f:
        data = f.read()
    fname  = os.path.basename(args.image)
    size   = len(data)
    ...
    meta_pkt = build_packet(PKT_META, 0, total,
                            encode_meta(size, total, CHUNK_BYTES, BLOCK_CHUNKS, md5_16, fname))
    print(f"[BLAST-TX] '{fname}' {size:,} B  {total} pkts  {MOD.upper()} — "
          f"looping META+DATA forever (Ctrl+C to stop)")
    sent = 0
    while True:
        tx_set(sdr, meta_pkt)                              # re-announce each pass
        time.sleep(TX_PACKET_BURST)
        for seq in range(total):
            tx_set(sdr, build_packet(PKT_DATA, seq, total, chunks[seq]))
            sent += 1
            print_progress((sent % total) + 1, total, "BLAST-TX", f"sent={sent}")
            time.sleep(TX_PACKET_BURST)


def blast_receiver(sdr):
    """Passive: decode every RX buffer, print a stats line every 2 s. No TX, no ARQ."""
    print(f"[BLAST-RX] Passive decode test ({MOD.upper()}). Stats every 2 s. Ctrl+C to stop.")
    t0 = time.time(); last = t0
    bufs = decoded = 0
    seen = {}                              # global DATA seq -> payload (dedup count)
    types = {}
    peak = 0.0
    while True:
        try:
            ...
            continue
        bufs += 1
        peak = max(peak, float(np.max(np.abs(raw))))
        for pkt in iq_to_packets(raw):
            decoded += 1
            types[pkt['type']] = types.get(pkt['type'], 0) + 1
            if pkt['type'] == PKT_DATA:
                seen[pkt['seq']] = pkt['payload']
        now = time.time()
        if now - last >= 2.0:
            adc = peak / 2896 * 100
            tdesc = " ".join(f"{hex(t)}:{n}" for t, n in sorted(types.items()))
            warn = " ⚠CLIP" if adc > 98 else ""
            print(f"[BLAST-RX] {now-t0:5.0f}s  bufs={bufs:4d}  decoded={decoded:5d}  "
                  f"uniqueDATA={len(seen):4d}  peak={peak:6.0f} ({adc:3.0f}%){warn}  "
                  f"types[{tdesc}]")
            last = now; peak = 0.0
...
def main():
    ...
    sdr = setup_pluto()
    print(f"\n*** ROLE: {ROLE.upper()}  "
          f"({'image SENDER' if ROLE=='tx' else 'image RECEIVER'})  MOD={MOD.upper()} ***")
    ...
    # ── Auto-calibration (unless skipped, or blast mode which can't handshake) ──
    if args.skip_cal or args.blast:
        if args.blast and not args.skip_cal:
            print("[*] --blast forces fixed gains (no bidirectional cal handshake).")
    ...
    try:
        if args.blast:
            blast_sender(sdr) if ROLE == 'tx' else blast_receiver(sdr)
        elif ROLE == 'tx':
            sender_main(sdr)
        else:
            receiver_main(sdr)
